...
dataset = pd.read_csv('../resources/mushrooms.csv', header=None)
...
def ID3(data, originaldata, features, target_attribute_name="target", parent_node_class=None):
    ...
    # If none of the above holds true, grow the tree!
        ...
        features = np.random.choice(features, size=int(np.sqrt(len(features))), replace=False)
...
def RandomForest_Predict(query, random_forest, default='p'):
    ...
    return np.unique(predictions)[0][0]
