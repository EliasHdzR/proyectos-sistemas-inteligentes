...
dataset = pd.read_csv('../resources/archive/zoo.csv',
...
def train_test_split(dataset):
    training_data = dataset.iloc[:90].reset_index(drop=True)  # We drop the index respectively relabel the index
    # starting form 0, because we do not want to run into errors regarding the row labels / indexes
    testing_data = dataset.iloc[90:].reset_index(drop=True)
